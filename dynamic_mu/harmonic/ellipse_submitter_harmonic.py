...
np.save(f"data_ellipses_phi_{phi:.2f}_v_{v0:.2f}_tau_{tau:.2f}_mu_{mu:.2f}.npy", data_ellipses, allow_pickle= True)
...
writervideo = FFMpegWriter(
    ...
    metadata={"artist": "capomav"},)
animation.save(f"morse_L_{L}_phi_{phi:.2f}_v0_{v0}_tau_{tau}_mu_{mu}.mp4", writer = writervideo, dpi=100)
#plt.show()
    ...
    for v0 in act_vels:
        for tau in taus:
            for mu in mus:
                    ...
                    wait_for_cpu_usage_below(threshold = 60, check_interval=100)
