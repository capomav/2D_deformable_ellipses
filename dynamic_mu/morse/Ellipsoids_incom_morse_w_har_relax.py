...
#constants 
phi = 0.8               #packing fraction; 0.005 for 1 particle
dt = 0.0001              #timestep
tot_time = 10
steps = int(tot_time/dt)
L = 20                #system size
v0 = 1.5               #active velocity
...
interval = 200      #used to be 250

relax_intervals = relax_steps//interval
run_intervals = steps//interval

total_steps = relax_intervals + run_intervals

traj_x = np.zeros((N,total_steps))
traj_y = np.zeros((N,total_steps))
traj_theta = np.zeros((N,total_steps))
traj_lmda_major = np.zeros((N,total_steps))
traj_lmda_minor = np.zeros((N,total_steps))
...
##### Harmomnic _calculations

@njit()
def force_12_harmonic(epsilon, beta, r, sigma, chi, sigma_0, chi_add, chi_sub, dots_add, dots_sub, rxi, u1xi, u2xi):
...
@njit()
def dUduxi_12_harmonic(epsilon, beta, r, sigma, sigma_0, chi, chi_add, chi_sub, dots_add, dots_sub, rxi, u1xi, u2xi):
...
@njit()
def calculate_force_torque_harmonic(rx, ry, r, chi, u1, u2):
    ...
    if r < sigma:
        ...
        Fx = force_12_harmonic(epsilon, beta, r, sigma, chi, sigma_0, chi_add, chi_sub, dots_add, dots_sub, rx, u1[0], u2[0])
        Fy = force_12_harmonic(epsilon, beta, r, sigma, chi, sigma_0, chi_add, chi_sub, dots_add, dots_sub, ry, u1[1], u2[1])

        dUdux = dUduxi_12_harmonic(epsilon, beta, r, sigma, sigma_0, chi, chi_add, chi_sub, dots_add, dots_sub, rx, u1[0], u2[0])
        dUduy = dUduxi_12_harmonic(epsilon, beta, r, sigma, sigma_0, chi, chi_add, chi_sub, dots_add, dots_sub, ry, u1[1], u2[1])
...
@njit()
def neighbour_forces_calc_harmonic(dis_x,dis_y, lmda_major, lmda_minor, dis_abs, u_vecs, neighbours, i):
    ...
    for j in neighbours:
        ...
        Fx_j, Fy_j, torque_j = calculate_force_torque_harmonic(dis_x[j], dis_y[j], dis_abs[j], chi_corrected, u_vecs[:,i], u_vecs[:,j])
        fx += Fx_j
        fy += Fy_j
        rcrossF += np.outer([dis_x[j], dis_y[j]], [Fx_j, Fy_j])
        torq += torque_j
...
@njit(parallel=True)
def update_harmonic(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, deformable, t, dt_step): #t is there for debug reasons
    ...
    for i in prange(N):
        ...
        fx,fy,rcrossF,torq = neighbour_forces_calc_harmonic(dis_x,dis_y, lmda_major, lmda_minor, dis_abs, u_vecs,neighbours,i) # Correct : add lambda major and add lambda minor
        Fx[i]+= fx
        Fy[i]+= fy
        torque_mech[i]+= torq
    ...
    if (deformable_relax):
         ...
         # defining the dynamic shear modulus
         delta_lmda = np.sqrt( (Lmda[0,0,:] + Lmda[1,1,:])**2 - 4*( Lmda[0,0,:]* Lmda[1,1,:] - Lmda[0,1,:]*Lmda[1,0,:] ) ) / (Lmda[0,0,:] + Lmda[1,1,:])
         mu_dynamic = mu*(1 + 50*delta_lmda**2)
         
         dLmda_dt[0,0,:] = -1/tau*(Lmda[0,0,:]-Lmda_0[0,0,:]) + R_0/(4*tau*mu_dynamic)*(stress[0,0,:] - stress[1,1,:])
         dLmda_dt[1,1,:] = -1/tau*(Lmda[1,1,:]-Lmda_0[1,1,:]) + R_0/(4*tau*mu_dynamic)*(stress[1,1,:] - stress[0,0,:])
         dLmda_dt[0,1,:] = -1/tau*(Lmda[0,1,:]-Lmda_0[0,1,:]) + R_0/(2*tau*mu_dynamic)*stress[0,1,:]
         dLmda_dt[1,0,:] = -1/tau*(Lmda[1,0,:]-Lmda_0[1,0,:]) + R_0/(2*tau*mu_dynamic)*stress[1,0,:]

         Lmda += dLmda_dt*dt_step
         ...
         theta += torque_mech*dt_step + (theta_def-theta) + np.sqrt(2*D_r*dt_step)*np.random.normal(0.0, 1.0, N)
         theta = periodic_boundaries(theta, 2*np.pi)
    else :
         theta += torque_mech*dt_step + np.sqrt(2*D_r*dt_step)*np.random.normal(0.0, 1.0, N)
         theta = periodic_boundaries(theta, 2*np.pi)
    ...
    x += (v0*np.cos(theta) + Fx)*dt_step + np.sqrt(2*D*dt_step)*np.random.normal(0.0, 1.0, N)
    x = periodic_boundaries(x, L)
    y += (v0*np.sin(theta) + Fy)*dt_step + np.sqrt(2*D*dt_step)*np.random.normal(0.0, 1.0, N)
...
@njit(parallel=True)
def update(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, deformable, t, dt_step): #t is there for debug reasons
...
def main(x,y, theta, Lmda, lmda_major, lmda_minor):
    for t in tqdm.tqdm(range(relax_steps)):
        x,y, theta, Lmda, lmda_major, lmda_minor = update_harmonic(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, deformable_relax, t, dt_relax)
    ...
    for t in tqdm.tqdm(range(steps)):
        x,y, theta, Lmda, lmda_major, lmda_minor = update(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, deformable, t, dt)
        
        if t%interval==0:
            traj_x[:,relax_intervals + t//interval] = x
            traj_y[:,relax_intervals + t//interval] = y
            traj_theta[:,t//interval] = theta
            traj_lmda_major[:,t//interval] = lmda_major
            traj_lmda_minor[:,t//interval] = lmda_minor
...
data_ellipses = {}
meta_data = {"phi":phi, "v_0": v0, "l_box": L, "D_r": D_r, "as_rat": aspect_ratio, "tau": tau, "K": K, "mu": mu}
data_ellipses["meta"] = meta_data
data_ellipses["x"] = traj_x
data_ellipses["y"] = traj_y
data_ellipses["orient"] = traj_theta
data_ellipses["l_major"] = traj_lmda_major
data_ellipses["l_minor"] = traj_lmda_minor

np.save(f"data_ellipses_phi_{phi:.2f}_v_0{v0:.2f}_tau{tau:.2f}_mu_{mu:.2f}.npy", data_ellipses, allow_pickle= True)
...
animation.save(f"morse_L_{L}_incom_phi_{phi:.2f}_mu_{mu}_tau_{tau}.mp4", writer = writervideo, dpi=100)

# frames to save and create vector image

frames_to_save = [50, 56] #range(len(traj_x))

for frame in frames_to_save:
     plot(frame)
     fig.savefig(os.path.join(os.getcwd(), f"frame_{frame}.svg"), dpi = 300, format = "svg")
