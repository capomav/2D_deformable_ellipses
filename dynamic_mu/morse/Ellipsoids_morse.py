...
R_0 = 1.0
sigma_0 = 2
aspect_ratio = 1.0      #ratio of major axis/minor axis; deformation only works if it is 1
...
n_save = (steps - 1)//interval + 1

traj_x = np.zeros((N, n_save))
traj_y = np.zeros((N, n_save))
traj_theta = np.zeros((N, n_save))
traj_lmda_major = np.zeros((N, n_save))
traj_lmda_minor = np.zeros((N, n_save))
...
def update(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, deformable, t, dt_step): #t is there for debug reasons
    ...
    if (deformable):    
         ...
         mu_dynamic = mu*(1 + 100*delta_lmda**2)
...
def main(x,y, theta, Lmda, lmda_major, lmda_minor):
    for t in tqdm.tqdm(range(relax_steps)):
        x,y, theta, Lmda, lmda_major, lmda_minor = update(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, False, t, dt_relax)
...
print("Checking saved trajectory...")

print("traj_x shape:", traj_x.shape)
print("traj_y shape:", traj_y.shape)
print("traj_theta shape:", traj_theta.shape)
print("traj_lmda_major shape:", traj_lmda_major.shape)
print("traj_lmda_minor shape:", traj_lmda_minor.shape)

print("NaN in x:", np.isnan(traj_x).any())
print("NaN in y:", np.isnan(traj_y).any())
print("NaN in theta:", np.isnan(traj_theta).any())
print("NaN in l_major:", np.isnan(traj_lmda_major).any())
print("NaN in l_minor:", np.isnan(traj_lmda_minor).any())

print("Inf in x:", np.isinf(traj_x).any())
print("Inf in y:", np.isinf(traj_y).any())
print("Inf in theta:", np.isinf(traj_theta).any())
print("Inf in l_major:", np.isinf(traj_lmda_major).any())
print("Inf in l_minor:", np.isinf(traj_lmda_minor).any())

print("min l_minor:", np.min(traj_lmda_minor))
print("max l_major:", np.max(traj_lmda_major))
print("max deformation ratio:", np.max(traj_lmda_major/traj_lmda_minor))
...
animation = FuncAnimation(fig, plot, frames=n_save, interval=500, blit=False, repeat=False)
...
animation.save(f"morse_L_{L}_phi_{phi:.2f}_v0_{v0}_K_{K}_mu_{mu}_tau_{tau}.mp4", writer = writervideo, dpi=100)
#plt.show()
