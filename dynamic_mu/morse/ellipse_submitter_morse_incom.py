...
deformable_relax = {{deform_relax_flag}}
...
interval = {{save_interval}}      #used to be 250
...
def update(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, deformable, t, dt_step): #t is there for debug reasons
    ...
    if (deformable):
         ...
         dLmda_dt[0,0,:] = -1/tau*(Lmda[0,0,:]-Lmda_0[0,0,:]) + R_0/(4*tau*mu_dynamic*K)*((mu_dynamic+K)*stress[0,0,:] + (mu_dynamic-K)*stress[1,1,:])
         dLmda_dt[1,1,:] = -1/tau*(Lmda[1,1,:]-Lmda_0[1,1,:]) + R_0/(4*tau*mu_dynamic*K)*((mu_dynamic+K)*stress[1,1,:] + (mu_dynamic-K)*stress[0,0,:])
...
def main(x,y, theta, Lmda, lmda_major, lmda_minor):
    for t in tqdm.tqdm(range(relax_steps)):
        x,y, theta, Lmda, lmda_major, lmda_minor = update_harmonic(x,y, theta, Lmda, lmda_major, lmda_minor, lmda_major_0, lmda_minor_0, False, t, dt_relax)
...
data_ellipses = {}
meta_data = {"phi":phi, "v_0": v0, "l_box": L, "D_r": D_r, "as_rat": aspect_ratio, "tau": tau, "K": K, "mu": mu, "sim_time":tot_time, "dt": dt}
...
np.save(f"data_ellipses_incom_phi_{phi:.2f}_v0_{v0:.2f}.npy", data_ellipses, allow_pickle= True)
...
norm = plt.Normalize(1, 1.5)  #since lmda1>=lmda2 by definition, min=1. max is chosen by feel
...
writervideo = FFMpegWriter(
    ...
    codec="libx264",
    extra_args=[
        "-crf", "23",          # 18–23 is common; lower = better quality
        "-preset", "fast",     # slower = better compression at same quality
        "-pix_fmt", "yuv420p"  # most compatible
    ],)
animation.save(f"morse_L_{L}_phi_{phi:.2f}_v0_{v0}.mp4", writer = writervideo, dpi=150)
#plt.show()
...
seeds = [275]
act_vels = [0.05, 0.1, 0.15, 0.2, 0.3, 0.4]
tau = 10.0
#mus = [0.1, 0.3, 0.5, 0.7, 1.0]
mus = [2.0]

phis = [0.35, 0.5, 0.65, 0.75, 0.80, 0.85]
boxl = params["boxl"]
    ...
    for v0 in act_vels:
        for phi in phis:
            for mu in mus:
                    ...
                    params["KV_mu"] = mu
                    params["phi"] = phi
                    params["movie_name"] = f"morse_L_{boxl}_phi_{phi:.2f}_v0_{v0}"
                    ...
                    wait_for_cpu_usage_below(threshold = 80, check_interval=60)
